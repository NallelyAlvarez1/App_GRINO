...
# -----------------------------------------------------------
# INTERFAZ VISUAL MEJORADA (CSS PREMIUM)
# -----------------------------------------------------------
...
<style>
    ...
    /* Reducir espacio y estilizar subheaders */
    h2, h3, h4 {
    ...
    /* Estilo de Tarjeta (Card) para las filas */
    div[data-testid="stDocContainer"] {
        padding: 0px !important;
    }
    
    /* Contenedor con borde personalizado */
    .element-container:has(.custom-card) {
        margin-bottom: -10px;
    }
    
    /* Etiquetas de Estado Modernas (Badges) */
    .badge {
        padding: 4px 10px;
        border-radius: 12px;
        font-size: 0.85rem;
    ...
    /* Estilo para los textos en negrita dentro de las tablas */
    .monto-destacado {
        font-size: 1.05rem;
        color: #1A1A1A;
        font-weight: 700;
...
with tab_presupuestos:
    ...
    if not presupuestos:
        ...
        # Encabezado plano y limpio
        with st.container():
            col1, col2, col3, col4, col5, col6, col7, col8 = st.columns([2, 2.3, 2.5, 0.9, 1.8, 1.8, 1, 3.2])
            col1.markdown("<small>**Cliente**</small>", unsafe_allow_html=True)
            col2.markdown("<small>**Lugar**</small>", unsafe_allow_html=True)
            col3.markdown("<small>**Descripción**</small>", unsafe_allow_html=True)
            col4.markdown("<small>**Ver.**</small>", unsafe_allow_html=True)
            col5.markdown("<small>**Fecha**</small>", unsafe_allow_html=True)
            col6.markdown("<small>**Total**</small>", unsafe_allow_html=True)
            col7.markdown("<small>**Ítems**</small>", unsafe_allow_html=True)
            col8.markdown("<small>**Acciones**</small>", unsafe_allow_html=True)

        for p in presupuestos:
            # Reemplazamos el contenedor estándar por uno con bordes e interactivo
            with st.container(border=True):
                ...
                if descripcion and descripcion != 'Sin descripción' and len(descripcion) > 28:
                    col3.write(descripcion[:25] + "...")
                else:
                    col3.write(descripcion)

                col4.write(f"v{notas}" if notas else "1")

                fecha_str = p.get('fecha_creacion', datetime.now().isoformat())
                col5.write(fecha_str.split('T')[0] if 'T' in fecha_str else fecha_str)
                    
                col6.markdown(f"<span class='monto-destacado'>${total_display:,.0f}</span>", unsafe_allow_html=True)
                col7.write(f"  {p.get('num_items', 0)}")
                ...
                with col8:
                    ...
                    with b2:
                        ...
                        if success and pdf_bytes:
                            st.download_button(label="⬇️", data=pdf_bytes, file_name=file_name, mime="application/pdf", key=f"down_{p['id']}", use_container_width=True, help="Descargar PDF")
                        else:
...
with tab_estados_cuenta:
    ...
    if not estados_cuenta:
        ...
        with st.container():
            col_cli, col_lug, col_fec, col_sub, col_abo, col_net, col_est, col_acc = st.columns([1.8, 1.8, 1.3, 1.3, 1.3, 1.4, 1.3, 2.8])
            col_cli.markdown("<small>**Cliente**</small>", unsafe_allow_html=True)
            col_lug.markdown("<small>**Lugar**</small>", unsafe_allow_html=True)
            col_fec.markdown("<small>**Emisión**</small>", unsafe_allow_html=True)
            col_sub.markdown("<small>**Monto Base**</small>", unsafe_allow_html=True)
            col_abo.markdown("<small>**Abonos**</small>", unsafe_allow_html=True)
            col_net.markdown("<small>**Total Neto**</small>", unsafe_allow_html=True)
            col_est.markdown("<small>**Estado**</small>", unsafe_allow_html=True)
            col_acc.markdown("<small>**Acciones**</small>", unsafe_allow_html=True)
        ...
        for ec in estados_cuenta:
            with st.container(border=True):
                ...
                col_net.markdown(f"<span class='monto-destacado'>${safe_numeric_value(ec.get('total_neto', 0)):,.0f}</span>", unsafe_allow_html=True)
                
                # Estado estilizado con Badges CSS profesionales
                es_pagado = ec.get('pagado', False)
                if es_pagado:
                    col_est.markdown('<span class="badge badge-pagado">🟢 Pagado</span>', unsafe_allow_html=True)
                else:
                    col_est.markdown('<span class="badge badge-pendiente">🔴 Pendiente</span>', unsafe_allow_html=True)
